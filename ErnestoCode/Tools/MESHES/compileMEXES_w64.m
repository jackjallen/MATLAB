function compileMEXES_w64(filename)
...
for i = 1:nFiles
    if( (~(strcmp('vtkPlantilla.cpp', filenames(i).name))) && ...
        ...
        if(nargin == 0)
            ...
            %eval(['mex -v ' filenames(i).name ' -output X:\MESHES_VTK\w64\' name '.mexw64']);
            eval(['mex -v ' filenames(i).name ' -output Z:\MESHES_VTK\' name '.mexw64']);
        else
