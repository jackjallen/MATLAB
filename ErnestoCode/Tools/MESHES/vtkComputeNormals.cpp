...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
  ...
  if( N->GetComputePointNormals() ) {
    n_xyz = N->GetOutput()->GetPointData()->GetArray("Normals")->GetNumberOfTuples();
    plhs[0]= mxCreateDoubleMatrix( n_xyz , 3 , mxREAL );

    for( p=0 ; p<n_xyz ; p++ ){
      *( mxGetPr( plhs[0] )+ p           )= N->GetOutput()->GetPointData()->GetArray("Normals")->GetComponent(p,0);
      *( mxGetPr( plhs[0] )+ p +   n_xyz )= N->GetOutput()->GetPointData()->GetArray("Normals")->GetComponent(p,1);
      *( mxGetPr( plhs[0] )+ p + 2*n_xyz )= N->GetOutput()->GetPointData()->GetArray("Normals")->GetComponent(p,2);
    }
  } else if( N->GetComputeCellNormals() ) {
    n_xyz = N->GetOutput()->GetCellData()->GetArray("Normals")->GetNumberOfTuples();
    plhs[0]= mxCreateDoubleMatrix( n_xyz , 3 , mxREAL );

    for( p=0 ; p<n_xyz ; p++ ){
      *( mxGetPr( plhs[0] )+ p           )= N->GetOutput()->GetCellData()->GetArray("Normals")->GetComponent(p,0);
      *( mxGetPr( plhs[0] )+ p +   n_xyz )= N->GetOutput()->GetCellData()->GetArray("Normals")->GetComponent(p,1);
      *( mxGetPr( plhs[0] )+ p + 2*n_xyz )= N->GetOutput()->GetCellData()->GetArray("Normals")->GetComponent(p,2);
    }
  } else {
    if( nlhs ){ plhs[0]= mxCreateDoubleMatrix( 0 , 0 , mxREAL ); }
  }
