/*
  phi = Euler4D( V , Gx , Gy , Gz , Gt , points , N , [t0 t1] ,
      ...
      size(V) = [ numel(Gx) numel(Gy) numel(Gz) numel(Gt)-1  3 ];
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { ALLOCATES();
  ...
  if( myNumel( prhs[4] ) != S ){ myErrMsgTxt("numel(Gt) Coordinates do not coincide with size(V,4)+1."); }
  ...
  for( p=0 ; p<nP ; p++ ){
    ...
    while( tau < T1 ){
      ...
      while( xyzl[0] >= X0 && xyzl[0] <= X1 &&
        ...
        v[0] =  ( U1*V1*W1*VX0000 - U1*V1*W0*VX0010
                + U1*V0*W0*VX0110 - U1*V0*W1*VX0100
                + U0*V1*W0*VX1010 - U0*V1*W1*VX1000
                + U0*V0*W1*VX1100 - U0*V0*W0*VX1110 )*D;

        v[1] =  ( U1*V1*W1*VY0000 - U1*V1*W0*VY0010
                + U1*V0*W0*VY0110 - U1*V0*W1*VY0100
                + U0*V1*W0*VY1010 - U0*V1*W1*VY1000
                + U0*V0*W1*VY1100 - U0*V0*W0*VY1110 )*D;

        v[2] =  ( U1*V1*W1*VZ0000 - U1*V1*W0*VZ0010
                + U1*V0*W0*VZ0110 - U1*V0*W1*VZ0100
                + U0*V1*W0*VZ1010 - U0*V1*W1*VZ1000
                + U0*V0*W1*VZ1100 - U0*V0*W0*VZ1110 )*D;
        ...
        if( computeJAC ){
          Vx_x =  ( V1*( W0*(VX0010-VX1010) + W1*(VX1000-VX0000)) + V0*( W0*(VX1110-VX0110) + W1*(VX0100-VX1100)) )*D*deltaT;
          Vy_x =  ( V1*( W0*(VY0010-VY1010) + W1*(VY1000-VY0000)) + V0*( W0*(VY1110-VY0110) + W1*(VY0100-VY1100)) )*D*deltaT;
          Vz_x =  ( V1*( W0*(VZ0010-VZ1010) + W1*(VZ1000-VZ0000)) + V0*( W0*(VZ1110-VZ0110) + W1*(VZ0100-VZ1100)) )*D*deltaT;

          Vx_y =  ( U1*( W0*(VX0010-VX0110) + W1*(VX0100-VX0000)) + U0*( W0*(VX1110-VX1010) + W1*(VX1000-VX1100)) )*D*deltaT;
          Vy_y =  ( U1*( W0*(VY0010-VY0110) + W1*(VY0100-VY0000)) + U0*( W0*(VY1110-VY1010) + W1*(VY1000-VY1100)) )*D*deltaT;
          Vz_y =  ( U1*( W0*(VZ0010-VZ0110) + W1*(VZ0100-VZ0000)) + U0*( W0*(VZ1110-VZ1010) + W1*(VZ1000-VZ1100)) )*D*deltaT;
          
          Vx_z =  ( U1*( V0*(VX0100-VX0110) + V1*(VX0010-VX0000)) + U0*( V1*(VX1000-VX1010) + V0*(VX1110-VX1100)) )*D*deltaT;
          Vy_z =  ( U1*( V0*(VY0100-VY0110) + V1*(VY0010-VY0000)) + U0*( V1*(VY1000-VY1010) + V0*(VY1110-VY1100)) )*D*deltaT;
          Vz_z =  ( U1*( V0*(VZ0100-VZ0110) + V1*(VZ0010-VZ0000)) + U0*( V1*(VZ1000-VZ1010) + V0*(VZ1110-VZ1100)) )*D*deltaT;
