...
#ifndef __cplusplus
#define myErrMsgTxt( ... )          mxErrMsgTxt( __VA_ARGS__ )
#else
...
#define mySize( mxA , i )           ( mxGetNumberOfDimensions( (mxA) ) > (i) ? (int) *( mxGetDimensions( (mxA) ) + i ) : 1 )
  ...
  #define TOC(s)                      QueryPerformanceCounter(   &__toc  );   \
                                      QueryPerformanceFrequency( &__frec );   \
                                      mexPrintf("%s .Elapsed time is %.8g seconds.\n", s, ( (double) (__toc.QuadPart - __tic.QuadPart) )/( (double) __frec.QuadPart )  )
  ...
  #define toc(i, s)                   tac(i);__tac##_##i = __tac##_##i/( (double) __frec##_##i.QuadPart );		\
									  mexPrintf("%s .Elapsed time is %.8g seconds.\n", s, ((double)__tac##_##i)  )
  ...
  #include <time.h>
  #define CreateTicToc_old()              clock_t     __tic_o, __toc_o; double dcpsec = 1 / ((double) CLOCKS_PER_SEC )
  #define TIC_old()                       __tic_o = clock()
  #define TOC_old(s)                      __toc_o = clock();          \
		mexPrintf("%s .Elapsed time is %.8g seconds.\n", s, ((double)( ( (double) (__toc_o - __tic_o) ) * dcpsec  )))

  
  #define CreateTicTacToc_old(i)          clock_t __tic_o##_##i, __toc_o##_##i; double __tac_o##_##i=0; double dcpsec = 1 / ((double) CLOCKS_PER_SEC )
  #define tic_old(i)                      __tic_o##_##i = clock()
  #define tac_old(i)                      __tac_o##_##i = __tac_o##_##i + ( (double) clock() - (double) __tic_o##_##i )
  #define toc_old(i,s)                    tac_old(i);__tac_o##_##i = __tac_o##_##i * dcpsec;		\
										  mexPrintf("%s .Elapsed time is %.8g seconds.\n", s, ((double)__tac_o##_##i)  )

/*Para tiempo mas preciso que con clock*/

  #include <stdint.h>
  #define CreateTicToc(i)		struct timespec __tic##_##i, __toc##_##i; double dtimeElapsed##_##i = 0;uint64_t timeElapsed##_##i = 0;
  #define TIC(i)				clock_gettime(CLOCK_MONOTONIC, &__tic##_##i)
  #define TOC(i, s)				clock_gettime(CLOCK_MONOTONIC, &__toc##_##i);          		\
								timeElapsed##_##i = ((__toc##_##i.tv_sec * 1e9) + 	\
								__toc##_##i.tv_nsec) - ((__tic##_##i.tv_sec * 1e9) + \
								__tic##_##i.tv_nsec); dtimeElapsed##_##i = timeElapsed##_##i;\
                                mexPrintf("%s .Elapsed time for timer %i is %g seconds.\n", s, i, (dtimeElapsed##_##i * 1e-9));

  #define CreateTicTacToc(i)	struct timespec __tic##_##i, __toc##_##i; double __tac##_##i=0; uint64_t timeElapsed##_##i = 0;
  #define tic(i)				clock_gettime(CLOCK_MONOTONIC, &__tic##_##i)
  #define tac(i)				clock_gettime(CLOCK_MONOTONIC, &__toc##_##i);				\
								timeElapsed##_##i = ((__toc##_##i.tv_sec * 1e9) + \
								__toc##_##i.tv_nsec) - ((__tic##_##i.tv_sec * 1e9) + \
								__tic##_##i.tv_nsec); __tac##_##i = __tac##_##i + timeElapsed##_##i;
  #define toc(i, s)				tac(i); mexPrintf("%s .Elapsed time for timer %i is %g seconds.\n", s, i, (__tac##_##i * 1e-9));
