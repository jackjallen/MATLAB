...
for d = data(1).DETERMINE_indices
    DETERMINE_dia_sys_dEPI2ENDOs(m,:) = [dia_dEPI2ENDOs(d,:)' ; sys_dEPI2ENDOs(d,:)']' ;

end
%%  PCA on myocardium thicknesses

dia_sys_dEPI2ENDOs_cov_mat = cov(MESA_dia_sys_dEPI2ENDOs);
...
MESA_dia_sys_dEPI2ENDOs_max_b = sqrt(principle_dia_sys_dEPI2ENDOs_eigenvalues);


%% PDM - find mean shape
dia_sys_dEPI2ENDOs_mean = mean(dia_sys_dEPI2ENDOs);
MESA_dia_sys_dEPI2ENDOs_mean = mean(MESA_dia_sys_dEPI2ENDOs);
% DETERMINE_dia_sys_dEPI2ENDOs_mean = mean(DETERMINE_dia_sys_dEPI2ENDOs);
...
for b = 1:nEigenvalues
    for i = data(1).DETERMINE_indices'
        for d = find(data(1).DETERMINE_indices==i)
            %'principle_dia_sys_dEPI2ENDOs_eigenvectors' from PCA on MESA
           DETERMINE_dia_sys_dEPI2ENDOs_b(d,b) = (principle_dia_sys_dEPI2ENDOs_eigenvectors(:,b)')*(dia_sys_dEPI2ENDOs(i,:)' - MESA_dia_sys_dEPI2ENDOs_mean');
            
        end
    end
    
    for i = data(1).MESA_indices'
        for m = find(data(1).MESA_indices==i)
            %         d
            MESA_dia_sys_dEPI2ENDOs_b(m,b) = (principle_dia_sys_dEPI2ENDOs_eigenvectors(:,b)')*(dia_sys_dEPI2ENDOs(i,:)' - MESA_dia_sys_dEPI2ENDOs_mean');
...
run('visualisemodes_diastole_myo_thicknesses_bullseyes_DM_res.m')
