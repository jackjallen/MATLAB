...
#
    ...
    if [ "$ENTRYPOINT" = "mexLibrary" ]; then
        MLIBS="-L$TMW_ROOT/bin/$Arch -lmx -lmex -lmat -lmwservices -lut -lrt"
    else  
        MLIBS="-L$TMW_ROOT/bin/$Arch -lmx -lmex -lmat -lrt"
    fi
    INCLUDE_DIRS_MIATOOLS=" -I/extra/disco1/miaTools/MESHES_VTK -I/extra/disco1/miaTools/Tools -I/extra/disco1/miaTools/GridData/include -I/usr/local/CARVE1.4.0/include/ "
    LIBS_MIATOOLS=" -L/extra/disco1/miaTools/Tools -lParseargs -ldl "
    INCLUDE_DIRS_VTK=" -I/usr/local/include/vtk-5.10 "
    INCLUDE_DIRS_CARVE=" -I/usr/local/include -I/usr/local/src/carve-1.4.0/include "
    #LIBS_VTK=" -Lvtk_libs -lvtkCommon -lvtkDICOMParser -lvtkexoIIc -lvtkexpat -lvtkFiltering -lvtkfreetype -lvtkftgl -lvtkGenericFiltering -lvtkGraphics -lvtkHybrid -lvtkImaging -lvtkIO -lvtkjpeg -lvtkNetCDF -lvtkpng -lvtkRendering -lvtksys -lvtktiff -lvtkVolumeRendering -lvtkWidgets -lvtkzlib "
    LIBS_VTK=" -Lvtk_libs -lvtkCommon -lvtkDICOMParser -lvtkexoIIc -lvtkexpat -lvtkFiltering -lvtkfreetype -lvtkftgl -lvtkGenericFiltering -lvtkGraphics -lvtkHybrid -lvtkImaging -lvtkIO -lvtkjpeg -lvtkNetCDF -lvtkpng -lvtkRendering -lvtksys -lvtktiff -lvtkVolumeRendering -lvtkWidgets -lvtkzlib "
    #LIBS_CARVE=" -Lcarve_libs -L/usr/local/CARVE1.4.0/common -lcarve -lcarve_fileformats -lcarve_misc  -lcarve_ui "
    LIBS_CARVE=" -Lcarve_libs -lcarve -lcarve_misc -lcarve_fileformats -lcarve_ui "
    case "$Arch" in
        Undetermined)
...
#----------------------------------------------------------------------------
            ...
            CFLAGS="$CFLAGS $INCLUDE_DIRS_MIATOOLS"
            CFLAGS="$CFLAGS $INCLUDE_DIRS_VTK"
            CFLAGS="$CFLAGS $INCLUDE_DIRS_CARVE"
            CLIBS="$RPATH $MLIBS -lm"
            COPTIMFLAGS='-O -DNDEBUG'
            CDEBUGFLAGS='-g'
            CLIBS="$CLIBS -lstdc++"
            CLIBS="$CLIBS $LIBS_MIATOOLS"
            CLIBS="$CLIBS $LIBS_VTK"
            CLIBS="$CLIBS $LIBS_CARVE"
#
            # C++keyName: GNU C++
            # C++keyManufacturer: GNU
            # C++keyLanguage: C++
            ...
            CXXFLAGS="$CXXFLAGS $INCLUDE_DIRS_MIATOOLS"
            CXXFLAGS="$CXXFLAGS $INCLUDE_DIRS_VTK"
            CXXFLAGS="$CXXFLAGS $INCLUDE_DIRS_CARVE"
            CXXLIBS="$RPATH $MLIBS -lm"
            CXXLIBS="$CXXLIBS $LIBS_MIATOOLS"
            CXXLIBS="$CXXLIBS $LIBS_VTK"
            CXXLIBS="$CXXLIBS $LIBS_CARVE"
            CXXOPTIMFLAGS='-O -DNDEBUG'
            CXXDEBUGFLAGS='-g'
#
            # FortrankeyName: gfortran
