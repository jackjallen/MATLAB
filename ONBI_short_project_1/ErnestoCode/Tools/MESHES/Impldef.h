...
//----------------------------------------------------------------------------
// Private Implemementation to hide STL.
//----------------------------------------------------------------------------
class Implem
{
public:
...
protected:
  Implem(const Implem&); // purposely not Implememented
  void operator=(const Implem&); // purposely not Implememented
            ...
            IntersectionMapType *map, vtkPolyData *interLines)
{
  ...
  while ( iter != ptIdMap.end() )
    ...
    ++iter;
     ...
     // PRUEBAS DAVID
//       if ((int)(cellId)>=16450)   // PRUEBAS DAVID
//       {fprintf(fid,"Celda=%i\n",(int)(cellId));
//        fprintf(fid,"Hasta aqui no\n");
//        fflush(fid);
//        fclose(fid);
//        return 0;
//       }  
