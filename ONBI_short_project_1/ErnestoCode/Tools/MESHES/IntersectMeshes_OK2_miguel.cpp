...
//   M->GetPoint( M->GetCell(cellid)->GetPointId(2) , Q );
  ...
  for( a = 3 ; a < nADDED ; a++ ){
    ...
    x = T[0][0]*(XYZ[0]-R[0])+T[0][1]*(XYZ[1]-R[1])+T[0][2]*(XYZ[2]-R[2])+rx;
    y = T[1][0]*(XYZ[0]-R[0])+T[1][1]*(XYZ[1]-R[1])+T[1][2]*(XYZ[2]-R[2])+ry;
   
    //mexPrintf("x=%.20f y=%.20f x+y=%.20f",x,y,x+y);
    
    if (x<rx) {
    	x=rx;
    //mexPrintf(" >> Corregido x");
    }
    if (y<ry) {
    	y=ry;
    	//mexPrintf(" >> Corregido y");
    }
    if ((x+y)>(rx+ry+1.0)) {
    	x=x-(x+y-rx-ry-1.0)/2.0;y=y-(x+y-rx-ry-1.0)/2.0;
    	//mexPrintf(" >> Corregido suma");
    }
    
    //mexPrintf(" >> x=%.20f y=%.20f x+y=%.20f\n",x,y,x+y);
