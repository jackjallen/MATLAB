...
Mex file of inpoly function of Darren Engwirda. Returns points laying inside/on bondary of a given polygon.
...
cnect  - Index of connected noded (default node  = [(1:Nnode-1) , Nnode ; (2:Nnode) , 1];
...
mex  -f mexopts_intel10.bat -output inpoly.dll inpoly.c
...
void mexFunction(int nlhs, mxArray** plhs ,  int nrhs, const mxArray** prhs)
{
	...
	int N = 0, N1, Nnode = 0, Ncnect;



	X         = mxGetPr(prhs[0]);  /* (2 x N) vector */

	if((mxGetM(prhs[0]) != 2) || (mxGetN(prhs[0]) == 0))
	{

		mxErrMsgTxt("X must be (2 x N), N >= 0");
	}
	...
	Xtemp     = (double *) mxMalloc(2*N*sizeof(double));
	...
	else
	{
		if(Nnode > 0)
		{
			...
			cnect    = mxMalloc(2*Ncnect*sizeof(double));
	...
	if(Nnode>0)
	{
		...
		nodetemp  = (double *) mxMalloc(2*Nnode*sizeof(double));
