...
%     end
    ...
    if ispc
        parfor whchain=1:nChains
            jagsScript   = sprintf( 'jagscript%d.cmd' , whchain );
            cmd = [ '"C:\Archivos de programa\JAGS\JAGS-3.4.0\i386\bin\jags.bat" ' jagsScript ];
%             cmd = sprintf( 'C:\Archivos de programa\JAGS\JAGS-3.4.0\i386\bin\jags.bat %s' , jagsScript );
            if verbosity > 0
    ...
    elseif ismac | isunix
        parfor whchain=1:nChains
            jagsScript   = sprintf( 'jagscript%d.cmd' , whchain );
            jagsPrefix = sprintf('export LD_LIBRARY_PATH=/usr/local/lib:/usr/lib:/usr/local/lib64:/usr/lib64; /usr/bin/');
