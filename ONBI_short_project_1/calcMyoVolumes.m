...
function[ myoVolumes ] = calcMyoVolumes(transformed_data)
...
cd C:\Users\jesu2687\Documents\MATLAB\ErnestoCode\
 ...
 % edges
test.diastolic.endo.B = vtkFeatureEdges( vtkCleanPolyData(transformed_data(2).diastolic.endo) , 'BoundaryEdgesOn' , [] , 'FeatureEdgesOff' , [] );  %extracting the boundary
test.diastolic.endo.B.xyz = test.diastolic.endo.B.xyz( [2 1 3:end], : );  %fixing the connectivity.

test.diastolic.epi.B = vtkFeatureEdges( vtkCleanPolyData(transformed_data(2).diastolic.epi) , 'BoundaryEdgesOn' , [] , 'FeatureEdgesOff' , [] );  %extracting the boundary
...
Last_prevID1  = size( data(2).diastolic.endo.xyz , 1 );
Last_prevID2  = size( data(2).diastolic.epi.xyz , 1 );
Last_prevID3 = size(myoB.xyz,1);
myoB_full.xyz = [ data(2).diastolic.endo.xyz ; myoB.xyz ; data(2).diastolic.epi.xyz ];
myoB_full.tri = [ data(2).diastolic.endo.tri; myoB.tri + Last_prevID1 ; data(2).diastolic.epi.tri + Last_prevID1 + Last_prevID3 ];
