function [data, dia_myo_mean, sys_myo_mean, dia_sys_myo_mean, MESA_diastolic_myo_shapes, DETERMINE_systolic_myo_shapes, all_training_diastolic_myo_shapes, all_training_systolic_myo_shapes , all_training_dia_sys_myo_shapes] = calcProcrustes(data, pI, dia_myo_reference, sys_myo_reference, dia_sys_myo_reference)
...
for p = 1:pI
    ...
    for n = 1:200
        ...
        [data(i).dia_sys.myo.prcrustes_d, dia_sys_myo_shapes(i).xyz, dia_sys_transform(i)] = procrustes(dia_sys_myo_reference, data(i).dia_sys.myo.xyz, 'scaling', false, 'reflection', false);
        ...
        for c = data(1).MESA_indices(:)'
            
            MESA_diastolic_myo_shapes(n,:) = all_training_diastolic_myo_shapes(c,:);
            MESA_systolic_myo_shapes(n,:) = all_training_systolic_myo_shapes(c,:);
            
            MESA_dia_sys_myo_shapes(n,:) =  all_training_dia_sys_myo_shapes(c,:);
            
        end
        for c = data(1).DETERMINE_indices(:)'
    ...
    dia_myo_mean = mean([MESA_diastolic_myo_shapes ; DETERMINE_diastolic_myo_shapes],1 );
    sys_myo_mean = mean([MESA_systolic_myo_shapes ; DETERMINE_systolic_myo_shapes] );
    
    dia_sys_myo_mean = mean([MESA_dia_sys_myo_shapes ; DETERMINE_dia_sys_myo_shapes ] ) ;
