...
%%
%match triangle face indicies to vertices
%face = pat1_endo(trifac(1,1),:)
pt1 = endo(trifac(i,1),:)
pt2 = endo(trifac(i,2),:)
pt3 = endo(trifac(i,3),:)
% max(trifac)
%Heron method for triangle area
%triangle face data is an index of the vertices that make up each face
%     a = sqrt((pt1(1)).^2 + (face_2 ).^2);
%     b = sqrt((pt2().^2 + (face_3).^2);
%     c = sqrt((face_3)).^2 + (pt1(1)).^2);
%     s = (a + b + c)/2; %semiperimeter
%     %triangle area (Heron's forumla)
